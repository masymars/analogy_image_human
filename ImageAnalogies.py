...
def read_image(filename, height, width):
    ...
    # Convert the resized image to a NumPy array of type float32 and normalize pixel values
...
def do_image_analogies(height,whidth,input_A_path, input_Ap_path, input_B_path,output_Bp_path, Kappa=0.0, NLevels=3, KCoarse=5, KFine=5, n_jobs=None, debugImages=False):
    A = read_image(input_A_path,height,whidth)
    Ap = read_image(input_Ap_path,height,whidth)
    B = read_image(input_B_path,height,whidth)
    # Make image pyramids
